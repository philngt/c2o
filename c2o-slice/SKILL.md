---
name: c2o-slice
description: Reduce a large feature, project, or specification to the smallest end-to-end vertical slice that delivers observable value and tests the riskiest useful assumption. Use when planned work crosses several layers or feels too large to implement and verify safely in one pass. Avoid horizontal slices that complete only one technical layer without a user-visible path.
---
...
4. Remove variants, optimization, automation, configuration, migration, polish, and generalized abstractions unless required for the path to work.
...
7. Confirm that completing it changes knowledge or value, not merely code volume.
